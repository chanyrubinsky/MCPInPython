...
# 4. ייבוא עם כינוי (Alias) כדי למנוע בלבול שמות
...
async def rewrite_letter(text: str, style: LetterStyle, add_emojis: bool) -> str:
    ...
    Rewrites a letter based on provided content and style.
    """
    # קריאה לפונקציה מתוך המודול gs (gemini_service)
    return await gs.draft_letter(text, style, add_emojis)

if __name__ == "__main__":
    # הפעלת השרת במצב SSE המאפשר חיבור דרך HTTP (כמו שהתוסף מצפה)
    mcp.run(transport="sse", host="127.0.0.1", port=5678)